...
const authorValidator = async function (req, res, next) {
    ...
    try {
        ...
        if(!data.fname) return res.status(400).send({status:false,msg:"enter fName"})
        ...
        if(!data.lname) return res.status(400).send({status:false,msg:"enter LName"})
        ...
        if(!data.email) return res.status(400).send({status:false, msg:"enter email id"})
